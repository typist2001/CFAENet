...
class TransformerDecoder_side(nn.Module):
    ...
    def __init__(self, dim_list=[768, 768, 384, 192], decoder_depths=[2, 6, 2, 2], num_heads=[24, 12, 6, 3],
        ...
        for i, item in enumerate(self.dims[:-1]):
            self.fusion.append(CAFEM(infeature=item, size=self.patches_resolution[3 - i]))
        self.fusion.append(CAFEM(infeature=self.dims[-1], size=self.patches_resolution[0]))
    ...
    def forward(self, rgb_features, depth_features):
        ...
        for i in range(len(self.layers)):
            # print(rgb_features[l-i].shape, depth_features[l-i].shape)
            if i == 0:
                rgbd_i = self.fusion[l - i - 1](rgb_features[l - i], depth_features[l - i], rgb_features[l - i])
                x = rgbd_i
            else:
                ...
                rgbd_i = self.fusion[l - i - 1](rgb_features[l - i], depth_features[l - i], x)
