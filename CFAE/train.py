...
from model.build_model import CFAENet
...
# load data
train_loader, train_num = get_loader(opt.rgb_root, opt.depth_root, opt.gt_root, opt.batchsize,
                                     opt.trainsize)
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, depths, gts) in enumerate(train_loader, start=1):
            ...
            loss_side = loss_s4 / 2 + loss_s3 / 4 + loss_s2 / 8
        ...
        torch.save(state, save_path + 'CFAENet_epoch_{}_checkpoint.pth.tar'.format(epoch + 1))
...
def val(val_loader, model, epoch, save_path):
    ...
    with torch.no_grad():
        ...
        if loss_epoch < best_loss:
            ...
            torch.save(model.state_dict(), save_path + 'CFAENet_best.pth')
