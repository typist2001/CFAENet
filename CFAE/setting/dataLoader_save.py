...
class SalObjDataset(data.Dataset):
    ...
    def __init__(self, image_root, depth_root, gt_root, edge_root, trainsize):
        """
...
def randomRotation(image, depth, gt, edge):
    mode = Image.BICUBIC
    if random.random() > 0.8:
        ...
        gt = edge.rotate(random_angle, mode)
    return image, depth, gt, edge


def get_loader(image_root, depth_root, gt_root, edge_root, batchsize, trainsize, shuffle=True, num_workers=6,
               pin_memory=True):
    dataset = SalObjDataset(image_root, depth_root, gt_root, edge_root, trainsize)
